...
public class Snipe extends BaseCommand {
    ...
    public void executePrefix(@NotNull MessageReceivedEvent event, String[] args) {
        try {
            ...
            event.getMessage().reply("No delete message found for the past 2 minutes").queue();
        }
    }
    ...
    public void executeSlash(@NotNull SlashCommandInteractionEvent event) {
        try {
            ...
            event.reply("No delete message found for the past 2 minutes").setEphemeral(true).queue();
        }
    }
}
